...
not climate-dependent — supporting our use of `ANOMALY.LEVEL` as a distinguishing
feature.
...
`CAUSE.CATEGORY` — utilities likely do not report customer counts for intentional
...
attack). We use median rather than mean because `OUTAGE.DURATION` is heavily
...
We evaluate using **weighted F1-score** rather than accuracy because the classes
